...
```
ISECO Facebook → rss.app RSS → Supabase Edge Function (poll_iseco_feed)
                                      ↓
...
- Accounts: Supabase, Firebase, Google AI (Gemini), rss.app
...
| `ISECO_RSS_FEED_URL` | [rss.app](https://rss.app/) — create feed for ISECO Facebook page |
...
supabase secrets set ISECO_RSS_FEED_URL=https://rss.app/feeds/...
...
| FCM not received | Confirm topic subscription; check `FCM_SERVER_KEY` |
| Parse failures | Inspect `parse_failures` table; retry with updated prompt |
| RSS has no images | Try different rss.app feed settings or carousel support |
